...
import ThrowHelper from './bowling/throw.helper';
...
describe('GameManager initialize', () => {
    ...
    it('Game Manager should be created with custom configuration', () => {
        ...
        let manager = new BowlingManager(customConfig);
    ...
    describe('Throw ball', () => {
        ...
        let throwRegular = (pins) => {
            sandbox.stub(ThrowHelper, 'getRandomThrowResult').returns(pins);
        ...
        let throwSpare = (throw1Pins = null, throw2Pins = null) => {
            sandbox.stub(ThrowHelper, 'getRandomThrowResult').returns(throw1Pins || [0, 1, 2]);
            manager.throwBall();
            sandbox.restore();
            sandbox.stub(ThrowHelper, 'getRandomThrowResult').returns(throw2Pins || [3, 4, 5, 6, 7, 8, 9]);
            let gameResult = manager.throwBall();
            sandbox.restore();
            return gameResult;
        }
